...
public class AreaServiceImpl implements AreaService {
    ...
    */
    ...
    @Override
    public List<AreaDto> findAllArea(String language, Optional<String> tipo) {
        /*
         ...
         */
        ProjectionOperation projectionOperation = Aggregation.project( "codice","regione","location","type").and(ConditionalOperators.ifNull(language+".nome").thenValueOf("it.nome")).as(language+".nome");
        ReplaceRootOperation replaceRootOperation = Aggregation.replaceRoot().withValueOf(ObjectOperators.valueOf(language).mergeWith(new Utils().createDocument("codice","regione","location","type","_id")));
        Aggregation aggregation = Aggregation.newAggregation(projectionOperation,replaceRootOperation);
        if(tipo.isPresent()){
            MatchOperation matchOperation = Aggregation.match(Criteria.where("type").is(tipo.get()));
            aggregation.getPipeline().add(matchOperation);
        }
    ...
    public AreaDto findById(String language,String id) {
        ...
        ProjectionOperation projectionOperation = Aggregation.project("codice","location","provincia","regione","enteGestore","type","luoghiDiInteresse").and(ConditionalOperators.ifNull(language).thenValueOf("it")).as(language);
        LookupOperation lookupOperation = Aggregation.lookup("luoghi_amp","codice","codice","luoghiDiInteresse");
        ReplaceRootOperation replaceRootOperation = Aggregation.replaceRoot().withValueOf(ObjectOperators.valueOf(language).mergeWith(new Utils().createDocument("codice","location","provincia","regione","enteGestore","type","coordinates","_id","luoghiDiInteresse")));
        ...
        Aggregation aggregation = Aggregation.newAggregation(matchOperation,lookupOperation,projectionOperation,replaceRootOperation);
        return mongoTemplate.aggregate(aggregation,mongoTemplate.getCollectionName(Area.class),AreaDto.class).getUniqueMappedResult();
...
*/
    ...
    public List<AreaDto> findAllAreePerRegione(String language,String regione) {
        ProjectionOperation projectionOperation = Aggregation.project("codice","type","location","regione").and(ConditionalOperators.ifNull(language+".nome").thenValueOf("it.nome")).as(language+".nome").and(ConditionalOperators.ifNull(language+".descrizione").thenValueOf("it.descrizione")).as(language+".descrizione");
        Criteria criteria = Criteria.where("regione").regex(regione,"i");
        MatchOperation matchOperation = Aggregation.match(criteria);
        ReplaceRootOperation replaceRootOperation = Aggregation.replaceRoot().withValueOf(ObjectOperators.valueOf(language).mergeWith(new Utils().createDocument("codice","nome","descrizione","type","_id","location","regione")));
